...
anti-pattern, and the driver would log a warning. Clients were encouraged to cache
`PreparedStatement` instances.

This cache is now built in: the driver will cache the first `prepare()` call and return the same
instance on subsequent invocations. See [prepared statements](../manual/core/statements/prepared/)
for more details. 
